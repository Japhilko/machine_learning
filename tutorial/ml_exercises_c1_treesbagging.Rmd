...
## [Plot the result](https://www.rdocumentation.org/packages/rpart.plot/versions/3.0.7/topics/rpart.plot)

```{r}
rpart.plot::rpart.plot(TREE,extra=1)
```


```{r,eval=F,echo=F}
rpart.plot::rpart.plot(TREE,type=5)
rpart.plot::rpart.plot(TREE,extra=1)
```
...
- The terminal nodes have 29, 12, 12, 3 and 8 observations
...
- `y` is a factor $\Rightarrow$ we choose `method="class"` 
```{r}
...
7) Which is the [**misclassification rate**](https://www.dataschool.io/simple-guide-to-confusion-matrix-terminology/) (prediction error) 
