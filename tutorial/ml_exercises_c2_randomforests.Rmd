...
16) Load the `ROCR` package. Use the `prediction()`, `performance()` and `plot()` command to print the [**ROC curve**](https://en.wikipedia.org/wiki/Receiver_operating_characteristic). Use `pred_dec_reg` variable from question 7. You can also refer to the previous exercise to see the code.
