...
\NormalTok{ames_train_hot <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(one_hot, ames_train) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{()}

\CommentTok{# make ranger compatible names}
\KeywordTok{names}\NormalTok{(ames_train_hot) <-}\StringTok{ }\KeywordTok{make.names}\NormalTok{(}\KeywordTok{names}\NormalTok{(ames_train_hot), }\DataTypeTok{allow_ =} \OtherTok{FALSE}\NormalTok{)}

\CommentTok{# hyperparameter grid search --> same as above but with increased mtry values}
...
\protect\hypertarget{how-to-proceed}{}
...
\KeywordTok{hist}\NormalTok{(OOB_RMSE, }\DataTypeTok{breaks =} \DecValTok{20}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{c2_random_forests_files/figure-beamer/unnamed-chunk-18-1.pdf}
