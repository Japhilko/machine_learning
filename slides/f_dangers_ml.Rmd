...
calc.sentiment <- function(input.text="This is great!", reg.model = fit.cv,
  ...
  n.unknown.words <- sum(!(word.vec %in% glove.df$word))
  ...
  ## }
    ...
    new.data <- as.matrix(glove.df[which(glove.df$word %in% word.vec),])
  }
...
#  return(mean(c(pred.val, rep.int(0, times=n.unknown.words))))
  return(mean(c(pred.val)))
}

## Need to make sure no NA's in dataset
##glove.df complete.cases(glove.df)
...
calc.sentiment("Let's go out for Italian food.", glove.df=glove.df)

calc.sentiment("Let's go out for Chinese food.", glove.df=glove.df)

calc. sentiment("Let's go out for Mexican food.", glove.df=glove.df)
