...
knitr::opts_chunk$set(echo = T,message=F,warning=F,cache=T,eval=F)
library(knitr)
```
...
Since the algorithm randomly selects 

- a bootstrap sample to train 
- and predictors to use at each split, tree correlation will be lessened beyond bagged trees.
...
- However, note that the best performing model has an OOB RMSE of 24504, which is lower than what we achieved previously. 
